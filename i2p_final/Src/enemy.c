...
bool updateEnemy(Enemy * enemy, Map * map, Player * player, BulletNode* bulletList){
    ...
    if(enemy->status == DYING){
        ...
        */
        ...
        if (enemy->death_animation_tick >= 32) {
			enemy->status = DEAD;
			map->enemy_counter--;
            return true;
        }
    ...
    if(enemy->knockback_CD > 0){
        ...
        */
        ...
        if (enemy->type == slime) {
		...
		else if (enemy->type == angry_slime) {
			if (playerCollision(enemy->coord, player->coord) && enemy->animation_hit_tick == 0) {
				enemy->animation_tick = 0;
				enemy->animation_hit_tick = 32;
				hitPlayer(player, enemy->coord, 10);
            ...
            //fireball
            enemy->fireball_timer++;
			if (enemy->fireball_timer >= FIREBALL_CD) {
				enemy->fireball_timer = 0;
                float angle = atan2(player->coord.y - enemy->coord.y, player->coord.x - enemy->coord.x);
                PointFloat center = (PointFloat){ enemy->coord.x + TILE_SIZE / 2, enemy->coord.y + TILE_SIZE / 2 };
                Bullet bullet = create_bullet("Assets/fireball.png", center, angle, 5, 5);
...
void drawEnemy(Enemy * enemy, Point cam){
    ...
    if(enemy->status == ALIVE){
        ...
        if(enemy->animation_hit_tick > 0 || enemy->type == angry_slime){
            offset += 16 * 8;
        }
		...
		//angry slime
        ALLEGRO_COLOR tint = enemy->knockback_CD > 0
        ...
        if (enemy->type == slime) {
			...
			al_draw_tinted_scaled_rotated_bitmap_region(enemy->image, offset, 0, 16, 16, tint,
				0, 0, dx, dy, TILE_SIZE / 16, TILE_SIZE / 16,
				0, flag);
		}
    }
        ...
        */
        int dying_offset = 16 * (int)(enemy->death_animation_tick / 4); 
        
        if (enemy->type == slime) {
            al_draw_tinted_scaled_rotated_bitmap_region(enemy->image, dying_offset, 16, 16, 16, al_map_rgb(255, 255, 255),
                0, 0, dx, dy, TILE_SIZE / 16, TILE_SIZE / 16,
                0, 0);
        }
		else if (enemy->type == angry_slime) {
			al_draw_tinted_scaled_rotated_bitmap_region(enemy->image, dying_offset, 16, 16, 16, al_map_rgba(255, 0, 0, 128),
				0, 0, dx, dy, TILE_SIZE / 16, TILE_SIZE / 16,
				0, 0);
		}
...
void updateEnemyList(enemyNode * dummyhead, Map * map, Player * player, BulletNode* bulletList){
    ...
    while(cur != NULL){
        bool shouldDelete = updateEnemy(&cur->enemy, map, player, bulletList);
        if(shouldDelete){
            prev->next = cur->next;
