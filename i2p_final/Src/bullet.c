...
bool update_bullet(Bullet * bullet, enemyNode * enemyList, Map * map, Player * player){
    /*
        [TODO Hackathon 3-1] 
    ...
    */
    ...
    //fireball
        ...
        Point bulletCoord = (Point){ (int)bullet->coord.x, (int)bullet->coord.y };
		if (bullet->coord.x >= player->coord.x &&
            bullet->coord.x <= player->coord.x + TILE_SIZE &&
            bullet->coord.y >= player->coord.y &&
            bullet->coord.y <= player->coord.y + TILE_SIZE) {
			hitPlayer(player, bulletCoord, bullet->damage);
			return true;
		}
	}
    return false;
...
void updateBulletList(BulletNode * dummyhead, enemyNode * enemyList, Map * map, Player * player){
    ...
    while(cur != NULL){
        bool shouldDelete = update_bullet(&cur->bullet, enemyList, map, player);
        if(shouldDelete){
            prev->next = cur->next;
